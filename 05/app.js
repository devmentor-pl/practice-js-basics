...
Student.prototype.getAverageGrade = function (sub) {
    if (!(this.grades[sub]) || this.grades[sub].length === 0) {
        return 'Brak ocen';
    } else {
        const avg = this.grades[sub].reduce((total, num) => total + num) / this.grades[sub].length;
        return avg;
    }
}
// ----------------PODDAJE SIĘ-------------------
...
const avgMath2 = student.getAverageGrade('geo');
console.log(`średnia ocen z przedmiotu to: ${avgMath2}`)

const avg = student.getAverageGradeOfAll();
